...
public class DatabaseManager implements IDataAccess{
    ...
    try {
        String databasePath = System.getProperty("user.dir") + File.separator + "database" + File.separator + "bibliotheque.db";

        System.out.println("📂 La base de données devrait être ici : " + databasePath);

        File dbFolder = new File(System.getProperty("user.dir") + File.separator + "database");
        if (!dbFolder.exists()) {
            dbFolder.mkdir();
            System.out.println("📁 Dossier 'database' créé !");
        }

        connection = DriverManager.getConnection("jdbc:sqlite:" + databasePath);
        System.out.println("✅ Connexion SQLite réussie : " + databasePath);
